...
fitted_knn_model <- nearest_neighbor() %>% 
  set_engine("kknn") %>% 
  set_mode("classification") %>%
  fit(letra ~., data = letras_train)
...
* En términos de performance, ambos modelos alcanzan un buen valor de accuracy ~0,7 indicando una buena performance del modelo. Sin embargo, cuando miramos el estadístico sensibilidad, este bajo ~0,3. Dado el des-balance de datos (mayor cantidad de noaes) el modelo focaliza su ajuste en esta clase.
* Pruebas preliminares en knn multi-clase (es decir, las vocales) arroja similar sesgo del modelo hacia la clase mas numerosa.
